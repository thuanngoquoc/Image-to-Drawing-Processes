...
class MainWindow(QMainWindow):
    def __init__(self, priority_map=None, dataset_slug="thuanngoquoc/inputdata"):
        ...
        if priority_map is None:
            priority_map = {
                # Ví dụ (bạn có thể bổ sung tuỳ ý):
        ...
        # Cấu trúc vẽ mới: each item = (object_label, part_name, [ [contour1], [contour2], ... ])
        ...
        # Các chỉ số để duyệt qua từng contour, từng điểm:
        self.current_obj_index = 0      # Đối tượng hiện tại
        self.current_contour_index = 0  # Contour hiện tại trong đối tượng
        self.current_point_index = 0    # Điểm hiện tại trong contour

        self.is_paused = False

        # Số pixel (hoặc điểm) muốn vẽ một lần
        # (có thể hiểu là "tốc độ" vẽ từng đoạn trong 1 lần update)
    ...
    def setup_drawing(self):
        thickness, ok = QInputDialog.getInt(self, "Drawing Thickness", 
                                            "Enter thickness of the line (>=1):", 
                                            value=1, min=1, max=20)
        if ok:
            self.line_thickness = thickness

        speed, ok = QInputDialog.getInt(self, "Drawing Speed", 
    ...
    def start_drawing(self):
        ...
        # Đếm số lần xuất hiện class để label chính xác (nếu cần)
        class_counts = {}
        for obj in objects:
        ...
        # Chuẩn bị list (label, part_name, list_of_contours)
        final_objects = []
        for obj in objects:
            c_name = obj["class_name"]
        ...
        # Xoá vùng sketch trên display_img trước khi vẽ
        h, w = self.pure_sketch.shape[:2]
        self.display_img[self.start_y:self.start_y+h, self.start_x:self.start_x+w] = (255, 255, 255)
        self.show_image(self.display_img, self.image_label)

        # Xây dựng danh sách vẽ self.all_drawing_instructions:
        # Mỗi phần tử: (object_label, part_name, [contour1, contour2, ...])
        # contourX là list các điểm (x, y) được sắp xếp theo thứ tự liên tiếp.
        self.all_drawing_instructions = []
        ...
        for label, parts in final_objects:
            for part_name, mask in parts:
                if mask is None:
                    continue
                # Lấy danh sách contour của part này
        ...
        # Ngoài ra, nếu muốn vẽ các vùng “remaining” (pixel đen chưa thuộc part nào),
        # ta có thể tìm contour còn sót lại. Ví dụ:
        remaining_mask = self.get_remaining_mask(pure_sketch_gray, objects)
        if remaining_mask is not None:
            contours_remain = self.extract_contours_in_order(remaining_mask, pure_sketch_gray)
            if contours_remain:
                self.all_drawing_instructions.append(("remaining_sketch", "remaining", contours_remain))
    ...
    def draw_next_line(self):
        """ Mỗi lần gọi, vẽ một (hoặc vài) đoạn line trong contour hiện tại. """
        ...
        if self.current_contour_index >= len(contour_list):
            # Chuyển sang part/object tiếp theo
        ...
        for _ in range(self.draw_batch_size):
            ...
            # Vẽ nét liền mạch
            cv2.line(self.display_img, (x1_display, y1_display),
                     (x2_display, y2_display), (0,0,0), thickness=self.line_thickness)

            self.current_point_index += 1

        # Sau khi vẽ xong batch_size đoạn, cập nhật hiển thị
        self.show_image(self.display_img, self.image_label)

    ######################################
    # Hàm tách lấy contour
    ######################################
    def extract_contours_in_order(self, mask, sketch_gray):
        """
        Tìm các contour (chu vi) của mask.
        Trả về list chứa nhiều contour, 
        mỗi contour là 1 list các điểm (x, y) liên tiếp.
        """
        # Tạo bản copy mask để dò contour
        mask_copy = mask.copy().astype(np.uint8)
        contours, _ = cv2.findContours(mask_copy, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

        contour_list = []
        for cnt in contours:
            # cnt là mảng shape (N,1,2) -> chuyển thành list (x, y)
            pts = [(p[0][0], p[0][1]) for p in cnt]
            # Bạn có thể sắp xếp hay xử lý thêm nếu muốn
            # Ở đây ta giữ nguyên thứ tự contour do opencv cung cấp
    ...
    def get_remaining_mask(self, sketch_gray, objects):
        """
        Ví dụ minh hoạ: tạo mask cho những vùng pixel đen 
        chưa thuộc bất kỳ part nào của object nào.
        Bạn có thể thay đổi logic tuỳ ý.
        """
        # Tạo mask từ sketch (chỗ đen < 50)
        base_mask = (sketch_gray < 50).astype(np.uint8)

        # Trừ dần các part mask
        for obj in objects:
            parts = obj["parts"].item() if isinstance(obj["parts"], np.ndarray) else obj["parts"]
